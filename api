...
def get_answer(model_name, user_message, system_prompt):
    ...
    answer, groq_tps = query_groq(groq_model, user_message, system_prompt)
    save_to_database(user_message, answer)
    return answer, False, groq_tps
...
def completions():
    ...
    answer, cached, tokens_per_second = get_answer(model_name, user_message, system_prompt)
    response_data = {
        "id": str(uuid.uuid4()),
